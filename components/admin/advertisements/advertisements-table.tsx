 ...
 * running.
 */
export function AdvertisementsTable({ ads, onPreview, onEdit, onDelete }: AdvertisementsTableProps) {
  if (ads.length === 0) {
    return (
      <EmptyState
      ...
      <table className="w-full min-w-[1000px] text-left text-sm">
        <thead className="sticky top-16 z-dropdown bg-card">
          <tr className="border-b border-border/60 text-xs uppercase tracking-wide text-muted-foreground">
            <th scope="col" className="py-3 pl-4 pr-4 font-medium">Advertisement Name</th>
            <th scope="col" className="py-3 pr-4 font-medium">Position</th>
            <th scope="col" className="py-3 pr-4 font-medium">Device</th>
            <th scope="col" className="py-3 pr-4 font-medium">Type</th>
            <th scope="col" className="py-3 pr-4 font-medium">Status</th>
            <th scope="col" className="py-3 pr-4 font-medium">Impressions</th>
            <th scope="col" className="py-3 pr-4 font-medium">Clicks</th>
            <th scope="col" className="py-3 pr-4 font-medium">CTR</th>
            <th scope="col" className="py-3 pr-4 font-medium">Created Date</th>
        ...
        <tbody className="divide-y divide-border/60">
          {ads.map((ad) => (
            <tr key={ad.id}>
              <td className="max-w-56 truncate py-3 pl-4 pr-4 font-medium text-foreground">{ad.name}</td>
              <td className="max-w-40 truncate py-3 pr-4 text-muted-foreground">{humanizeEnumValue(ad.position)}</td>
              ...
              <td className="py-3 pr-4 text-muted-foreground">{computeCtr(ad.impressions, ad.clicks)}%</td>
