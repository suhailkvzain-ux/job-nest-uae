...
export async function generateMetadata({ params }: CompanyDetailPageProps): Promise<Metadata> {
  ...
  if (!company) {
    return constructMetadata({ title: "Company Not Found", noIndex: true, path: `/companies/${slug}` });
  }
...
export default async function CompanyDetailPage({ params, searchParams }: CompanyDetailPageProps) {
  ...
  const [results, totalJobs, relatedCompanies] = await Promise.all([
    ...
    <>
      <JsonLd
        data={breadcrumbJsonLd([
          ...
          { name: "Companies", url: `${siteConfig.url}/companies` },
          { name: company.name, url: `${siteConfig.url}${basePath}` },
        ])}
      />
